...
/// <summary>Global hotkeys via RegisterHotKey on a never-shown top-level Form (so it also receives the broadcast "show panel" message); ids map to actions.</summary>
sealed class HotkeyManager : Form
{
    ...
    {
        Text = "NetPaw.Hotkeys"; ShowInTaskbar = false; FormBorderStyle = FormBorderStyle.None; Opacity = 0; StartPosition = FormStartPosition.Manual; Location = new Point(-32000, -32000); Size = new Size(1, 1);
        _ = Handle; // create the window now; it is never shown
    }
