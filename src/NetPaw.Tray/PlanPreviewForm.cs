...
{
    PlanPreviewForm(ApplyPlan plan, string? subtitle, bool canApply)
    {
        ...
        var apply = new Button { Text = plan.IsEmpty ? "Nothing to do" : "Apply", Width = 120, Height = 30, DialogResult = DialogResult.OK, Enabled = canApply && !plan.IsEmpty, Visible = canApply };
        var cancel = new Button { Text = canApply ? "Cancel" : "Close", Width = 100, Height = 30, DialogResult = DialogResult.Cancel };
    ...
    {
        using var f = new PlanPreviewForm(plan, subtitle, canApply: true);
        return f.ShowDialog() == DialogResult.OK;
    }

    public static void ShowReadOnly(ApplyPlan plan, string? subtitle = null)
    {
        using var f = new PlanPreviewForm(plan, subtitle, canApply: false);
        f.ShowDialog();
    }
}
